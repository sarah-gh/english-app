...
<template>
  <div class="min-h-screen bg-background px-4 py-6 pb-18.75">
    ...
    <template v-else>
      <p v-if="quizSessionStore.isSubmitted" class="mb-6 rounded-lg border-2 border-primary/30 p-4 text-center">
        <span class="text-2xl font-semibold text-primary">{{ quizSessionStore.score }} / {{
          quizSessionStore.questions.length }}</span>
      ...
      <div class="space-y-5">
        <BaseCard v-for="(question, index) in quizSessionStore.questions" :key="question.id">
          <div class="mb-2 flex items-center justify-between">
            ...
            <span
v-if="quizSessionStore.isSubmitted" class="inline-flex items-center gap-1 text-xs font-semibold"
          ...
          <div v-if="question.type === 'multiple-choice' && question.options" class="space-y-2">
            <label
...
v-else :model-value="quizSessionStore.answers[question.id] ?? ''"
            :disabled="quizSessionStore.isSubmitted" placeholder="Type your answer…"
...
v-if="quizSessionStore.isSubmitted && !quizSessionStore.isCorrect(question)"
          ...
          <BaseButton
v-if="quizSessionStore.isSubmitted" variant="ghost" size="sm"

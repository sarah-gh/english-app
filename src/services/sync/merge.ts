 ...
 */
export function mergeSingleton<T extends { updatedAt: number }>(
  local: T,
  remote: T | undefined,
): SingletonMergeResult<T> {
 ...
 */
function deduplicateByKey<T extends DedupableEntity>(
  entities: T[],
  ...
  for (const group of groups.values()) {
    ...
    const mostRecentlyEdited = group.reduce((latest, entity) =>
...
export function deduplicateByName<T extends NamedSyncEntity>(
  ...
  return deduplicateByKey(entities, (entity) =>
    options.scopeKey
      ? `${options.scopeKey(entity)}::${normalizeName(entity.name)}`

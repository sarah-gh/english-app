...
async function getAccessToken(options: {
  ...
  const isUsable = (
    token: AccessTokenResult | null,
    requiredMargin: number,
  ): token is AccessTokenResult => token !== null && token.expiresAt - requiredMargin > Date.now();

  const cached = cachedToken;
  if (!options.forceRefresh && isUsable(cached, margin)) return cached.accessToken;

  if (!options.allowWindow) {
    throw new SyncAuthError('Cloud Sync is paused until your next sync.', 'refresh_required');
  }
...
export async function syncNow(
  ...
  try {
    ...
    await Promise.all([
      ...
      settingsResult.toApplyLocally
        ? settingsRepository.replace(settingsResult.merged)
        : Promise.resolve(),
    ]);

...
export function buildDescriptiveEvaluationPrompt(
  ...
  return `You are grading an English-language learner's answers to an open-ended practice quiz. For EACH item below, evaluate how well the learner's answer demonstrates correct understanding and usage, score generously for genuine understanding even with minor grammar slips, but score low for answers that are off-topic, incorrect, or blank. Set "sourceIndex" to the item's number (1-based) shown below. Give a "score" from 0 to 100: award a full 100 ONLY when the answer fully satisfies every requirement of the question with no meaningful errors; if ANY requirement is missed or ANY inaccuracy is present, the score MUST be below 100. Build "feedback" as a short (2-4 sentence) explanation directed at the learner; if "score" is below 100, "feedback" MUST also include, after a blank line, a section starting with the exact heading "Why points were deducted:" that lists the specific reasons (e.g. a requested item from the prompt is missing, a grammatical inaccuracy, an inaccurate or incomplete explanation, a weak or generic example). Do not include that heading when "score" is 100. Give a "sampleAnswer" showing what a strong answer would look like. STRICT LANGUAGE REQUIREMENT: "feedback" and "sampleAnswer" MUST be written entirely in English, never a Persian/Farsi translation or any other non-English text.${proficiencyInstruction(proficiencyLevel)}

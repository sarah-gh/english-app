...
export const cardRepository = {
  ...
  async getByDeck(deckId: string): Promise<Card[]> {
    return byCreatedAtDesc(tombstones.live(await db.cards.where('deckId').equals(deckId).toArray()));
  },

  async getByTopic(topicId: string): Promise<Card[]> {
    return byCreatedAtDesc(tombstones.live(await db.cards.where('topicId').equals(topicId).toArray()));
  },
  ...
  async getByReviewStatus(status: ReviewStatus): Promise<Card[]> {
    return byCreatedAtDesc(tombstones.live(await db.cards.where('reviewStatus').equals(status).toArray()));
  },
  ...
  async decrementStudyCount(id: string): Promise<void> {
    const card = await db.cards.get(id);
    if (!card) return;
    await db.cards.update(id, { studyCount: Math.max(0, card.studyCount - 1), updatedAt: Date.now() });
  },

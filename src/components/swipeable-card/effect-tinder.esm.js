export default function EffectTinder({ swiper, on }) {
  ...
  const activeSlideEl = () =>
    isVirtual()
      ? swiper.slides.find(
          (el) => parseInt(el.getAttribute('data-swiper-slide-index'), 10) === swiper.activeIndex,
        )
  ...
  const setSlideButtonsLabels = (slideEl, progress, diff, isLastSlide, force) => {
  ...
  on('touchEnd', () => {
    ...
    if (draggableSlide) {
      if (Math.abs(draggableSlide.progress) < 0.1 || activeSlideEl() === draggableSlide) {
  ...
  on('slidesGridLengthChange', () => {
    const allSlidesLength = isVirtual() ? swiper.virtual.slides.length : swiper.slides.length;

    const isLastSlide = swiper.activeIndex === allSlidesLength - 1 && !swiper.params.loop;
  ...
  on('setTranslate', (s, currentTranslate) => {
    ...
    swipeDirection = diff;
    const { slides } = swiper;
    const allSlidesLength = isVirtual() ? swiper.virtual.slides.length : slides.length;
    ...
    slides.forEach((slideEl, slideIndex) => {
      ...
      if (
        (slideEl.tinderTransform && slideEl.tinderTransformSlideIndex !== slideIndex) ||
        !isTouched
      ) {
        slideEl.tinderTransform = '';
      }
      slideEl.style.zIndex = -Math.abs(Math.round(slideProgress)) + allSlidesLength;

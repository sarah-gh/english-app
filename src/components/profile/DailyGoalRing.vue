  ...
  <div class="flex flex-col items-center gap-2 rounded-xl border border-text/10 bg-white/80 dark:bg-slate-900/80 p-4">
    <div class="relative h-28 w-28">
      <svg viewBox="0 0 100 100" class="h-full w-full -rotate-90">
        <circle cx="50" cy="50" :r="RADIUS" fill="none" stroke="currentColor" stroke-width="8" class="text-text/10" />
        <circle
cx="50" cy="50" :r="RADIUS" fill="none" stroke="currentColor" stroke-width="8" stroke-linecap="round"
    ...
    <div v-if="isEditingGoal" class="flex items-center gap-2">
      <input
v-model="goalDraft" type="number" min="1"
    ...
    <button
v-else type="button" class="text-xs text-text/40 underline underline-offset-2 hover:text-primary"
      @click="startEditGoal">
      Edit goal
    </button>
  </div>
